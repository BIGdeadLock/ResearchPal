...
def _add_chunks_metadata(chunks: list[Chunk], metadata: dict) -> dict:
    for chunk in chunks:
        ...
        if "requesters" not in metadata[category]:
            metadata[category]["requesters"] = list()

        metadata[category]["num_chunks"] = metadata[category].get("num_chunks", 0) + 1
        metadata[category]["requesters"].append(chunk.requester_full_name)
...
def _add_embeddings_metadata(embedded_chunks: list[EmbeddedChunk], metadata: dict) -> dict:
    for embedded_chunk in embedded_chunks:
        ...
        metadata[category]["requesters"].append(embedded_chunk.requester_full_name)
